...
import {
  ...
  Shield, Building, Mail, Power,
  RefreshCw, CheckCircle2, AlertCircle
} from 'lucide-react';

const NAV = [
  { id: 'managers', icon: <Users size={18} />,   label: 'Managers' },
  { id: 'create',   icon: <UserPlus size={18} />, label: 'Add Manager' },
...
export const Dashboard = () => {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      notify(`Manager "${form.name}" created successfully`);
      setForm({ name: '', password: '', email: '' });
      setTab('managers');
      fetchManagers();
    } catch (err) {
      notify(err.response?.data?.message || err.message || 'Failed to create manager', 'error');
    } finally {
      setCreating(false);
    }
  };

  const fetchManagers = async (pageNum = page) => {
    setLoadingMgr(true);
    try {
      const data = await adminService.getEmployees(pageNum, 5);
      setManagers(data?.content ?? (Array.isArray(data) ? data : []));
  ...
  useEffect(() => { fetchManagers(page); }, [page]);

  const handleToggle = async (id) => {
    try {
      const msg = await adminService.toggleEmployeeStatus(id);
  ...
  const saveEdit = async (m) => {
    try {
      ...
      notify(err.response?.data?.message || 'Failed to update manager', 'error');
    }
  };
        ...
        {tab === 'managers' && (
          <div className="fade-up">
            <div style={{
              ...
              {[
                { label: 'Total',    value: managers.length, color: '#0e7490' },
                { label: 'Active',   value: managers.filter(m => m.enabled !== false).length, color: '#047857' },
                { label: 'Inactive', value: managers.filter(m => m.enabled === false).length, color: '#b91c1c' },
              ].map((s, i) => (
            ...
            <div className="win">
              <div style={{ padding: '13px 20px', borderBottom: '1px solid rgba(180,200,220,0.28)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '0.86rem', fontWeight: 600, color: 'var(--t1)' }}>Managers</span>
                <button className="btn btn-ghost" style={{ padding: '5px 11px', fontSize: '0.78rem' }} onClick={() => fetchManagers()}>
                  <RefreshCw size={13} style={loadingMgr ? { animation: 'spin 0.7s linear infinite' } : {}} /> Refresh
                </button>
              </div>
              <div style={{ overflowX: 'auto' }}>
                {managers.length === 0 ? (
                  <div style={{ padding: '48px 24px', textAlign: 'center', color: 'var(--t3)', fontSize: '0.86rem' }}>
                    {loadingMgr ? 'Loading…' : 'No managers found.'}
                  </div>
                ) : (
        ...
        {tab === 'create' && (
          <div className="fade-up win" style={{ maxWidth: 460, margin: '0 auto' }}>
            <div style={{ padding: '14px 20px', borderBottom: '1px solid rgba(180,200,220,0.28)', display: 'flex', alignItems: 'center', gap: 8 }}>
              <UserPlus size={15} color="#7e22ce" />
              <span style={{ fontSize: '0.86rem', fontWeight: 600, color: 'var(--t1)' }}>New Manager</span>
            </div>
              ...
              <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                  <button type="button" className="btn btn-ghost" style={{ flex: 1 }}
                    onClick={() => setTab('managers')}>Cancel</button>
            ...
            {isEditingProfile ? (
              ...
              <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                {[
                  ...
                  { label: 'Role',     value: 'Administrator', icon: <Shield size={14} color="#7e22ce" /> },
                ].map((row, i) => (
                  <div key={i} style={{
                    display: 'flex',
                    justifyContent: 'space-between',
